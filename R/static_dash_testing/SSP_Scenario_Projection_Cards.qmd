...
sst_scenario_comparison <- ggplot(ssp_sst_wide) +
  ...
  theme(legend.box = "horizontal",
        ...
        legend.position.inside = c(0.25, .81),
...
us_ssp85_sst_projections <- ggplot() +
  ...
  scale_color_manual(values = c("black", gmri_cols("lv orange"))) +
...
both_scenario_comparison <- ggplot(ssp_sst_wide) +
  geom_line(
    data = ssp_sst_wide %>% filter(scenario == "SSP1: 2.6"),
    aes(year, `Ensemble Mean`, color = scenario),
    linewidth = 1, key_glyph = "timeseries", linetype = 1) +
  geom_ribbon(
    aes(year, ymin = `5th Percentile`, ymax = `95th Percentile`, fill = scenario ),
    color = "transparent", alpha = 0.2) +
  geom_line(
  ...
  geom_line(
    data = obs_annual, 
    aes(year, surf_temp, color = "Observed"),
    linewidth = 1.5, key_glyph = "timeseries") +
  # Threshold Points and error
  geom_point(
    data = label_df,
    aes(x, y), shape = "|", size = 4) +
  geom_text(
  ...
  guides(
    color = guide_legend(
      order = 1, ncol = 1, title.position = "top", title.hjust = 0.5),
    fill = guide_legend(
      order = 2, title.position = "top", title.hjust = 0)) +
  theme_gmri() +
  labs(fill = "Range in Ensemble Uncertainty\n(5th-95th Percentiles)", 
       color = "SST", 
       x = "Year", 
       y = str_c("Average Annual Sea Surface Temperature"),
       subtitle = "Northeast U.S.",
...
#both_scenario_comparison


# showtext::showtext_opts(dpi=300)
# ggsave(
#   plot = both_scenario_comparison,
#   filename = here::here("R/static_dash_testing/dash_img/both_scenarios_compare_sst.png"),
#   dpi = 300,
#   bg = "white",
#   width = 5.4,
#   height = 2.5,
...
![](dash_img/both_scenarios_compare_sst.png)
...
# Get the difference
diff_df <- get_difference(
  base_dat = st_drop_geometry(base_df), 
  proj_dat = st_drop_geometry(projected_df))
...
# Load the trawl survey area
us_area <- read_sf(str_c(cs_path("res", "Shapefiles/BottomTrawlStrata/"), "BTS_Strata.shp"))  %>% 
  janitor::clean_names() %>% 
  filter(strata >= 01010 ,
         strata <= 01760,
         strata != 1310,
  ...
  st_union() %>% 
  st_as_sf() %>% 
  mutate(region = "US Survey Zone")

# What coordinates/hexagons touch?
hex_us <- hex_grid %>% 
  st_join(
    y = st_transform(us_area, st_crs(hex_grid)),
    join = st_intersects) %>% 
  drop_na(region) %>% 
  st_drop_geometry()
...
# Use them to filter the data going into the plotting functions:
# easy
```

```{r}
#| label: observed-baseline-us
#| eval: false


# Make a map of the baseline distribution
haddock_baseline_map_us <- ssp_proj_map(
  dist_df = filter(base_df, pt_id %in% hex_us$pt_id),
  reactive = F, 
  add_labels = F) + 
  coord_sf(
    xlim = c(-200000, 1050000), 
    ylim = c(3875000, 5200000) , 
    expand = F, crs = 32619) +
  labs(title = "Haddock Average Biomass Density (2010-2019)")
...
# # showtext::showtext_opts(dpi=300)
# ggsave(
#   plot = haddock_baseline_map_us,
#   filename = here::here("R/static_dash_testing/dash_img/haddock_baseline_plot_us.png"),
#   dpi = 300,
#   bg = "white",
#   width = 4,
#   height = 4,
...
```{r}
#| label: observed-diff-us
#| eval: false

# Make a map of the baseline distribution
#ssp_proj_map(dist_df = projected_df, reactive = F, add_labels = F)
haddock_change_map_us <- ssp_difference_map(
  dist_df = filter(diff_df, pt_id %in% hex_us$pt_id),
  reactive = F,
  range_shift = F) + 
  coord_sf(
    xlim = c(-200000, 1050000), 
...
# showtext::showtext_opts(dpi=300)
# ggsave(
#   plot = haddock_change_map_us,
#   filename = here::here("R/static_dash_testing/dash_img/haddock_change_plot_us.png"),
#   dpi = 300,
#   bg = "white",
#   width = 4,
#   height = 4,
