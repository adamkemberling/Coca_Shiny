...
bathy <- terra::rast(here::here("COCA_SDM_app_dev/dev/scratch_data", "NEShelf_Etopo1_bathy.tiff"))


# Load the hexagonal grid
...
clean_annual_wide <- read_csv(
  ...
  file = here::here("COCA_SDM_app_dev/dev/projections/annual_proj_wide.csv"),
  col_types = cols(
    pt_id = col_double(),
...
clean_season_wide <- read_csv(
  ...
  file = here::here("COCA_SDM_app_dev/dev/projections/seasonal_proj_wide.csv"),
  col_types = cols(
    pt_id = col_double(),
    species = col_character(),
    scenario = col_character(),
