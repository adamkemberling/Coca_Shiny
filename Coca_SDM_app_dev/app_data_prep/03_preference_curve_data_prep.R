...
# Which was moved into coca_shiny under: Baseline_Env_Conditions.qmd

horizon_conditions <- read_csv(
  #here::here("Data/env_data/regional_scenario_conditions_at_horizons.csv"),
  here::here("Coca_SDM_app_dev/dev/scratch_data/regional_scenario_conditions_at_horizons.csv"),
  col_types = list(
...
pref_slim <- pref_all %>% 
  select(comname, val_z = Value, val_actual, fit_exp, variable)
# only need: val_actual, fit exp, comname
...
####  Save Them  ####
write_csv(pref_slim, 
          here::here("Coca_SDM_app_dev/app_ready_data/preference_curve_data.csv"))
write_csv(horizon_conditions, 
          here::here("Coca_SDM_app_dev/app_ready_data/projected_environmental_conditions.csv"))
